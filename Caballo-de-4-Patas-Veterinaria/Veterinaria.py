...
class VeterinariaApp:
    def __init__(self, root):
        ...
        ctk.CTkButton(boton_frame, text="GESTIONAR MASCOTAS", fg_color="#16a085", text_color="white",
                    width=160, command=self.abrir_gestion_ventana, corner_radius=0).pack(side="left", padx=10)
        ctk.CTkButton(boton_frame, text="HISTORIAL MÉDICO", fg_color="#8e44ad", text_color="white",
                    width=160, command=self.abrir_historial_ventana, corner_radius=0).pack(side="left", padx=10)

    def abrir_historial_ventana(self):
        VentanaHistorialMascota(self.root, self.controller)

    def abrir_gestion_ventana(self):
        VentanaGestionMascotas(self.root, self.controller)
