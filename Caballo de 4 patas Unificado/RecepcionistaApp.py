...
class GestionHorasApp:
    ...
    def agendar_hora(self):
        ...
        try:
            fecha_hora = datetime.strptime(f"{fecha} {hora}", "%Y-%m-%d %H:%M").isoformat()
        except ValueError:
        ...
        try:
            # Obtener cliente
            resp = requests.get(f"http://127.0.0.1:8000/recepcionista/clientes/{rut_cliente}")
        ...
        # Obtener mascota
            resp = requests.get(f"http://127.0.0.1:8000/veterinaria/mascotas/{nombre_mascota}")
            if resp.status_code != 200:
                messagebox.showerror("Error", "Mascotas del cliente no encontradas.")
                return
            m = resp.json()
            if m["nombre"].lower() != nombre_mascota.lower():
                messagebox.showerror("Error", "Mascota no coincide.")
                return
            mascota = m
            if not mascota:
    ...
    # NOTA: Rut del veterinario y recepcionista se asumen como IDs directos (provisional)
    # Si usas un router separado como usuarios_router, debes consultar allí.
    # Aquí lo dejamos manual:
            id_vet = int(rut_vet)  
            id_recepcionista = int(rut_recep)
    ...
    # Crear consulta
            ...
            resp = requests.post("http://127.0.0.1:8000/recepcionista/consultas/", json=consulta_data)
            if resp.status_code == 200:
                messagebox.showinfo("Éxito", "Consulta agendada correctamente.")
                self.limpiar_campos_formulario()
                self.cargar_horas()
            else:
                messagebox.showerror("Error", f"No se pudo agendar la consulta. {resp.text}")
